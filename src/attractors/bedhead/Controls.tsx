...
import { bedheadData } from "../../Parametersets";

interface BedheadControlsProps {
  params: BedheadParams;
  onChange: (params: BedheadParams) => void;
  disabled?: boolean;
  selectedPreset: number;
  onPresetChange: (index: number) => void;
}

const presetOptions = bedheadData.map((preset, index) => ({
  value: index,
  label: preset.name,
}));

export const BedheadControls: React.FC<BedheadControlsProps> = ({
  params,
  onChange,
  disabled = false,
  selectedPreset,
  ...
  const handleChange = (key: keyof BedheadParams) => (value: number) => {
...
export default BedheadControls;
