...
import { jasonRampe2Data } from "../../Parametersets";

interface JasonRampe2ControlsProps {
  params: JasonRampe2Params;
  onChange: (params: JasonRampe2Params) => void;
  disabled?: boolean;
  selectedPreset?: number;
  onPresetChange?: (index: number) => void;
}

const presetOptions = jasonRampe2Data.map((preset, index) => ({
  value: index,
  label: preset.name,
}));

export const JasonRampe2Controls: React.FC<JasonRampe2ControlsProps> = ({
  params,
  onChange,
  disabled = false,
  selectedPreset = 0,
  ...
  const handleChange = (key: keyof JasonRampe2Params) => (value: number) => {
...
export default JasonRampe2Controls;
