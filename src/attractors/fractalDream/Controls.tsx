...
import { fractalDreamData } from "../../Parametersets";

interface FractalDreamControlsProps {
  params: FractalDreamParams;
  onChange: (params: FractalDreamParams) => void;
  disabled?: boolean;
  selectedPreset: number;
  onPresetChange: (index: number) => void;
}

const presetOptions = fractalDreamData.map((preset, index) => ({
  value: index,
  label: preset.name,
}));

export const FractalDreamControls: React.FC<FractalDreamControlsProps> = ({
  params,
  onChange,
  disabled = false,
  selectedPreset,
  ...
  const handleChange = (key: keyof FractalDreamParams) => (value: number) => {
...
export default FractalDreamControls;
